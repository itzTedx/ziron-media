...
const ServicesMarquee = dynamic(
	() =>
		import("@/components/bento/services-marquee").then(
			(mod) => mod.ServicesMarquee
		),
	{
			...
			<div className="container flex flex-col items-center gap-6 max-md:px-3">
				...
				<BentoGrid>
					{features.map((feature, idx) => (
						<BentoCard key={Number(idx)} {...feature} />
					))}
...
const features = [
	...
	},
	{
		...
		background: (
			<div className="mt-28">
				<AnimatedList className="mask-[linear-gradient(to_top,transparent_10%,#000_100%)] absolute top-0 right-2 h-[300px] w-full border-none transition-all duration-300 ease-out group-hover:scale-105 md:top-4" />
			</div>
		),
	},
	{
	...
	},
	{
		...
		background: (
			<div className="mt-28">
				<AnimatedBeam className="absolute -top-4 border-none transition-all duration-300 ease-out md:top-4 md:right-2 md:h-[350px] md:group-hover:scale-105" />
			</div>
		),
	},
];
