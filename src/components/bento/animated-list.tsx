				...
				>
					<div className="flex flex-row items-center gap-2">
						...
						<div className="flex flex-col overflow-hidden">
							<figcaption className="flex flex-row items-center whitespace-pre font-medium text-lg">
							...
							<p className="truncate font-normal text-xs">{description}</p>
						</div>
					</div>
				</figure>
			);
		...
		>
			<AnimatedList>
				{baseNotifications.map((item, idx) => (
					<Notification {...item} index={idx} key={Number(idx)} />
				))}
			</AnimatedList>
		</div>
	);
