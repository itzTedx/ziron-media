/** biome-ignore-all lint/suspicious/noExplicitAny: we don't know the type of the children */
"use client";
				...
				{scrollSnaps.map(
					(
						...
						<button
							...
							key={`${carouselId}-${Number(snap)}`}
							onClick={() => onDotButtonClick(index)}
							type="button"
						>
							<div className="size-1.5 rounded-full bg-gray-500/40" />
							{index === selectedIndex && (
								<AnimatePresence mode="wait">
									<motion.div
										className="absolute top-0 left-0 z-[3] h-full w-full rounded-full bg-secondary"
