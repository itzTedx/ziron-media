...
export default function Cta() {
	return (
		<section
			className="relative mx-auto max-w-6xl overflow-hidden pt-40 pb-24 max-md:px-4"
			id="cta"
		>
			<div className="relative flex h-72 w-full items-center justify-between gap-12 rounded-[1.875rem] border border-secondary bg-gradient-to-bl from-gray-900 to-violet-900 shadow-primary-md md:h-96">
				<div className="relative z-40 w-full px-5 md:px-12">
					<h4 className="pb-9 font-medium text-2xl text-purple-50 leading-10 drop-shadow-2xl max-md:text-end md:text-[2.375rem] md:text-purple-200">
						Take your marketing
						<br />
						to the next level
					</h4>
					...
					>
						<div className="group relative w-36 cursor-pointer overflow-hidden rounded-full border border-white bg-background/50 p-3 text-center font-semibold text-background backdrop-blur-md max-md:ml-auto">
							...
							<div className="absolute top-0 left-0 z-10 flex h-full w-full translate-y-12 items-center justify-center gap-2 rounded-full bg-primary text-background opacity-0 transition-all duration-300 group-hover:translate-y-0 group-hover:rounded-none group-hover:opacity-100">
			...
			<Blob className="absolute top-1/2 right-0 -z-40 -translate-y-1/2 max-md:opacity-50 md:scale-125" />
		</section>
	);
}
