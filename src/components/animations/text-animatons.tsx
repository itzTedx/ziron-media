...
const TextAnimate: FC<Props> = ({
	...
	if (type === "rollIn" || type === "whipIn") {
		return (
			<h2 className="mt-10 px-8 py-5 pb-8 font-black text-3xl text-black md:text-5xl dark:text-neutral-100">
	...
	return (
		<motion.h2
			animate="visible"
			className="mt-10 px-8 py-5 pb-8 font-black text-4xl text-black md:text-5xl dark:text-neutral-100"
			initial="hidden"
			role="heading"
		...
		>
			{letters.map((letter, index) => (
				<motion.span key={`${letter}-${Number(index + 1)}`} variants={child}>
					{letter === " " ? "\u00A0" : letter}
				</motion.span>
			))}
		</motion.h2>
